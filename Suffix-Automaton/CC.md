...
Suppose <i>δ<sub>w</sub>(w<sub>k</sub>, a) = ß</i>, i.e. following the transition from state <i>p</i> with letter <i>a</i> leads to state <i>suf = [w<sub>k</sub> • a]<sub>w</sub></i>. We have to consider two cases:
* <i>ß = [w<sub>k</sub> • a]<sub>w</sub></i>  
In this case we can simply assign <i>slink([wa]<sub>wa</sub>) = suf</i>
* <i>ß ╪ [w<sub>k</sub> • a]<sub>w</sub></i>  
In this case, since after extending <i>w</i> with the letter <i>a</i> the string <i>w<sub>k</sub> • a</i> occurs in two distinct left contexts, a new state, <i>clone = [w<sub>k</sub> • a]<sub>wa</sub></i>, has to be created. The creation of a new state requires updating the suffix link chain. Since <i>slink([wa]<sub>wa</sub>) = [w<sub>k</sub> • a]<sub>wa</sub> = clone</i>, this implies that ß is not a suffix of <i>wa</i>.  
