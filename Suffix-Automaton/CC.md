...
<b><i>Proof:</i></b> Let <i>Φ' = Φ U {1, 2, ..., n}</i> and let <i>S = Φ \ {{Ø}, {1, 2, ..., n}}</i>. Then there exists <i>S' c Φ': S c S'</i>. Denote by <i>p(S)</i> the smalles set such that: <i>S c p(S)</i> and <i>S ╪ p(S)</i>. In a sense <i>p(S)</i> is the parent of <i>S</i>. We will show that <i>(Φ' \ {Ø}, p, {1, 2, ..., n})</i> is a rooted tree with root <i>{1, 2, ..., n}</i>, parent fuction <i>p</i> and set of states <i>Q = Φ' \ {Ø}</i>.
