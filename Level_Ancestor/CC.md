...
Querying the structure is performed at two steps. Given a query <i>LA<sub>T</sub>(v, k)</i> first we check whether the height of node <i>v</i> is greater or smaller than <i>B</i>.  
  ...
  * If <i>h<sub>v</sub> &le; B</i> then <i>v</i> is a micro node and we store a pointer to the root of the micro tree <i>T<sub>micro<sub>k</sub></sub></i> that it belongs to <i>r<sub>v</sub> = root(v)</i>.  
